...
Bitboard Position::slider_blockers(Bitboard sliders, Square s, Bitboard& pinners, Color c) const {
  ...
  else
  {
      for (PieceSet ps = piece_types(); ps;)
      {
          ...
          if (b)
          {
              ...
              if (AttackRiderTypes[pt] & asymmetricalRides)
              {
  ...
  snipers &= ~slidingSnipers; // leave hoppers

  while (slidingSnipers)
  {
    Square sniperSq = pop_lsb(slidingSnipers);
  ...
  while (snipers)
  {
    ...
    Bitboard b = between_bb(s, sniperSq, type_of(piece_on(sniperSq))) & (pieces() ^ sniperSq);

    if (b && popcount(b) == 2)
    {
        if(AttackRiderTypes[piece_on(sniperSq)] & RIDER_GRASSHOPPER) {
            Bitboard b2 = b & PseudoAttacks[WHITE][KING][s];
            if(b2) b &= ~2; else b = 0;
        } else
        // Janggi cannons block each other
        if ((pieces(JANGGI_CANNON) & sniperSq) && (pieces(JANGGI_CANNON) & b)) {
            b &= pieces(JANGGI_CANNON);
            if(more_than_one(b)) b = 0;
        }
        blockers |= b;
        if (b & pieces(color_of(piece_on(s))))
            pinners |= sniperSq;
...
Bitboard Position::attackers_to(Square s, Bitboard occupied, Color c, Bitboard janggiCannons) const {
  ...
  for (PieceSet ps = piece_types(); ps;)
  {
      ...
      if (board_bb(c, pt) & s)
      {
          ...
          // Consider asymmetrical moves (e.g., horse)
          if (AttackRiderTypes[move_pt] & asymmetricalRides)
          {
...
bool Position::gives_check(Move m) const {
      ...
      && !((var->petrifyOnCaptureTypes & type_of(moved_piece(m))) && capture(m)))
  {
      ...
      else if (AttackRiderTypes[pt] & (HOPPING_RIDERS | asymmetricalRides) || nonSlidingTypes & pt)
      {
...
if(type_of(captured) == KING){
  sync_cout << nonSlidingTypes << " " << Bitboards::pretty(st->previous->nonSlidingRiders) << sync_endl;
  std::string t = std::to_string(from_sq(st->previous->move)) + "-" + std::to_string(to_sq(st->previous->move)) + "\n";
  t += std::to_string(from) + "-" + std::to_string(to) + "\n";
  for(Square s = SQ_A1; s < SQUARE_NB; ++s) {
    t += std::to_string(piece_on(s)) + " ";
    if(file_of(s) == FILE_L) t += "\n";
  }
